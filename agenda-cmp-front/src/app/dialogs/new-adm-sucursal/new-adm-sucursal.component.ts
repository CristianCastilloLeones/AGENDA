...
@Component({
  ...
  templateUrl: "./new-adm-sucursal.component.html",
  styleUrls: ["./new-adm-sucursal.component.css"],
})
export class NewAdmSucursalComponent implements OnInit {
  admForm: FormGroup;
  role: any = [];
  processing = false;
  ...
  constructor(
    ...
    private dialogRef: MatDialogRef<NewAdmSucursalComponent>,
    ...
    this.admForm = this.formBuilder.group({
      ...
      role: new FormControl({ value: [9], disabled: false }, [
        Validators.required,
      ]),
  ...
  createUser() {
    this.userService.createAdmSucursal(this.admForm.value).subscribe(
      (res) => {
        this.processing = false;
        new SwalComponent({
          type: "success",
